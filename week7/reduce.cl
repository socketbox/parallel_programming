kernel void ArrayMultReduce( global const float *dA, global const float *dB, local float *prods, global float *dC )
{
	...
	/* multiply the two arrays together and add them up. come up with one sum per work-group
	it is a big performance benefit to do it here while “prods” is still available – and is local it 
	would be a performance hit to pass prods back to the host then bring it back to the device for reduction*/
